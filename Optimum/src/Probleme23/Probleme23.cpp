...
{
    float *Ecart;                                               // Tableau contenant l'ecart entre les différents points
    ...
    Result.Fmin = def_fonction(m_Xmin) ;                        // Initialisation de la plus petite valeur
    for (unsigned int i=0; i<m_NbreVariable; i=i+1)
        {
            Result.P[i] = m_Xmin [i];
        }

        for (unsigned int i=0; i<m_NbreVariable; i=i+1)        // Calcul de tous les écarts pour chaque dimension et stockage dans un tableau
        {
            Ecart[i] = (m_Xmax [i] - m_Xmin [i])/9;
        }

    switch(m_NbreVariable)                                     // J'ai programmé en fonction de la dimension de l'espace de recherche
    {
    case 2:                                                    // Dimension de l'espace de recherche égale à 2

        for (unsigned int i=0; i<9; i=i+1)                     // On se balade dans la première dimension
        {
            x[0] = m_Xmin [0]+ i*Ecart[0];
            for (unsigned int j=0; j<9; j=j +1)                // On se balade dans la seconde dimension
            {
              x[1] = m_Xmin [1] + j*Ecart[1];
              f = def_fonction(x);
              if (f < Result.Fmin)                             // Recherche du minimum
              {
                Result.Fmin = f;
        ...
        break;
    case 3:                                                    // Dimension de l'espace de recherche égale à 3
        for (unsigned int i=0; i<9; i=i+1)                     // On se balade dans la première dimension
        {
            x[0] = m_Xmin [0]+ i*Ecart[0];
            for (unsigned int j=0; j<9; j=j +1)                // On se balade dans la seconde dimension
            {
              x[1] = m_Xmin [1] + j*Ecart[1];
              for (unsigned int k=0; k<9; k=k +1)              // On se balade dans la troisième dimension
                {
